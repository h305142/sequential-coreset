...
def data_syn(d=20):
    ...
    print(np.mean(np.power(np.dot(X, h) - y, 2)) + 10000 * np.sum(np.abs(h)) / X.shape[0])
    return X, y, h
...
def FGD(max_iter, X,y,weight,h,lamda=lamd,lr=0.01):
    ...
    also PGD of FGD
    """
    threshold = lr * lamda /np.sum(weight)
...
def l2_loss(X,y,beta,lamda=lamd):
    loss=np.sum(np.power(np.dot(X,beta)-y,2),0)/X.shape[0]+np.sum(lamda*np.abs(beta),0)/X.shape[0]
    return loss


def l2_loss_coreset(X,y,beta,weight,lamda=lamd):
    loss=np.sum(weight*(np.power(np.dot(X,beta)-y,2)),0)/np.sum(weight)+np.sum(lamda*np.abs(beta),0)/np.sum(weight)
    return loss
    ...
    for d in [50]:
        ...
        for n in n_range:
            ...
            for i in range(10):
                ...
                s = time.time()
                beta_coreset3, iter6 = lr_coreset_run(X, y, optimal=optimal[opt], R_ball=1, lamda=lamda,
                                                      epoch_max=1000)
                timing.append(time.time() - s)
                ...
                loss_train.append([l2_loss_test_reg(X, y, beta_origin, lamda=lamda),
                                   l2_loss_test_reg(X, y, beta_layer, lamda=lamda),
                                   l2_loss_test_reg(X, y, beta_uniform, lamda=lamda),
                                   l2_loss_test_reg(X, y, beta_coreset2, lamda=lamda),
                                   l2_loss_test_reg(X, y, beta_coreset3, lamda=lamda),
                                   l2_loss_test_reg(X, y, beta_coreset7, lamda=lamda),
                                   l2_loss_test_reg(X, y, beta_coreset8, lamda=lamda),
                                   l2_loss_test_reg(X, y, beta_near, lamda=lamda)])
            Iter = np.mean(iter, 0)
        ...
        path = 'result/test/Lasso/'
        if not os.path.exists(path):
            os.makedirs(path)
        scio.savemat(path + 'coreset_Loss_lr01_cvge-6_{}.mat'.format(n_range[0]),
