...
def FGD(max_iter, X,y,weight,h,lamda=lamd,lr=0.01):
    for iter in range(max_iter):
        gradient_f = (np.sum(weight*(2*(X*(np.dot(X, h) - y))),0)[:, np.newaxis]+2*lamda*h)/sum(weight)
        h -=lr*gradient_f
    return h
...
def LMS_run(X,y,optimal,lamda=lamd,epoch_max=1000):
    ...
    l_before = l2_loss_coreset(coreset_x, coreset_y, beta, weight, lamda)
    for iter in range(max_iter, max_iter * epoch_max, max_iter):
        beta = optimal(max_iter, coreset_x, coreset_y,weight, beta, lamda=lamda,
                             lr=lr2)
        # l_now =l2_loss(X, y, beta,lamda)
        # l_now = l2_loss_coreset(coreset_x, coreset_y, beta, weight, lamda)
        # if (np.abs(l_now - l_before) < epsilon):
...
def l2_loss_test_reg(X_test, y_test, beta, lamda=lamd):
...
def main():
    ...
    for n in [1e3,1e4]:
        ...
        for d in range(5,16):
            ...
            iter, beta_res = [], []
            beta_diff = []
            time_1, flag, time_LMS, flag_LMS = 0, 1, 0, 1
            ...
            for i in range(10):
                ...
                if flag_LMS == 1:
                    print("LMS:")
                    beta_LMS, iter13 = LMS_run(X, y, optimal=optimal[opt], lamda=lamda, epoch_max=1000)
                    time_LMS = time.time() - s
                    timing.append(time_LMS)
                    flag_LMS = 2
                else:
                    timing.append(time_LMS)
                tttime.append(timing)
                iter.append([iter1, iter3, iter4, iter5, iter6, iter7,iter13])
                beta_res.append(
                    [beta_origin, beta_layer, beta_uniform, beta_coreset2, beta_coreset3,
                     beta_coreset7, beta_coreset8, beta_near,beta_LMS])
                ...
                loss.append([l2_loss_test(X_test, y_test, beta_origin, lamda=lamda),
                             l2_loss_test(X_test, y_test, beta_layer, lamda=lamda),
                             l2_loss_test(X_test, y_test, beta_uniform, lamda=lamda),
                             ...
                             l2_loss_test(X_test, y_test, beta_LMS, lamda=lamda)])
                loss_train.append([l2_loss_test_reg(X_test, y_test, beta_origin, lamda=lamda),
                                   l2_loss_test_reg(X_test, y_test, beta_layer, lamda=lamda),
                                   l2_loss_test_reg(X_test, y_test, beta_uniform, lamda=lamda),
                                   l2_loss_test_reg(X_test, y_test, beta_coreset2, lamda=lamda),
                                   l2_loss_test_reg(X_test, y_test, beta_coreset3, lamda=lamda),
                                   l2_loss_test_reg(X_test, y_test,beta_coreset7, lamda=lamda),
                                   l2_loss_test_reg(X_test, y_test, beta_coreset8, lamda=lamda),
                                   l2_loss_test_reg(X_test, y_test, beta_near, lamda=lamda),
                                   l2_loss_test_reg(X_test, y_test, beta_LMS, lamda=lamda)])
            Iter = np.mean(iter, 0)
        ...
        if not os.path.exists(path):
            os.mkdir(path)
        scio.savemat(path + 'coreset_5_d_lr01_cvge-4_{}_size_{}.mat'.format( opt_path[opt],n),
                     {'Time': np.array(Time), 'Loss_avg': np.array(Loss), 'Loss_var': np.array(Loss_var),
                      ...
                      'Loss_train_avg': np.array(Loss_train_avg),
